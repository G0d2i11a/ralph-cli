import { Task } from '../types/task';

export function buildFinalizeRetryReset(): Partial<Task> {
  return {
    lastErrorSignature: undefined,
    lastErrorHadObjectiveProgress: undefined,
    finalizerCommitMessage: undefined,
    finalizerCommitSha: undefined,
    finalizerCommittedAt: undefined,
    finalizerAttempts: 0,
    finalizerFailure: undefined,
    repairContext: undefined,
    mergedAt: undefined,
    integratedAt: undefined,
    integrationStatus: 'not_started',
    integrationCommitSha: undefined,
    integrationBranch: undefined,
    integrationWorktree: undefined,
    targetSyncedAt: undefined,
    targetSyncStatus: 'not_requested',
    targetSyncDeferredReason: undefined,
    mergeCommitSha: undefined,
    mergeTargetBranch: undefined,
    mergeStrategy: undefined,
    mergeMessage: undefined,
    mergeError: undefined,
    mergeConflictFiles: undefined,
    mergeConflictPhase: undefined,
    mergeConflictAt: undefined,
    mergeRepairAttempts: 0,
    mergeRepairBranch: undefined,
    mergeRepairWorktree: undefined,
    finalizeRepairStartedAt: undefined,
    finalizeRepairDeadlineAt: undefined,
    finalizeRepairLastFailureSnapshot: undefined,
    finalizeRepairLastProgressAt: undefined,
    finalizeRepairLastProgressReason: undefined,
    finalizeRepairConsecutiveNoProgress: 0,
    finalizeRepairTotalRequeues: 0,
    finalizeRepairStoppedAt: undefined,
    finalizeRepairStopReason: undefined,
    autoRecoveryKind: undefined,
    autoRecoveryTotalRequeues: 0,
    autoRecoveryHardCap: undefined,
    autoRecoveryLastRequeuedAt: undefined,
    autoRecoveryNextEligibleAt: undefined,
    autoRecoveryStoppedAt: undefined,
    autoRecoveryStopReason: undefined,
    autoRecoveryLastReason: undefined,
    failedBlockerRecoveryStartedAt: undefined,
    failedBlockerRecoveryDeadlineAt: undefined,
    failedBlockerRecoveryTotalRequeues: 0,
    failedBlockerRecoveryLastSignature: undefined,
    failedBlockerRecoveryStoppedAt: undefined,
    failedBlockerRecoveryStopReason: undefined,
    failedBlockerRecoveryDemandTaskIds: undefined,
    storyRepairRecoveryStartedAt: undefined,
    storyRepairRecoveryDeadlineAt: undefined,
    storyRepairRecoveryTotalRequeues: 0,
    storyRepairRecoveryLastSignature: undefined,
    storyRepairRecoveryConsecutiveSameSignature: 0,
    storyRepairRecoveryStoppedAt: undefined,
    storyRepairRecoveryStopReason: undefined,
    storyRepairRecoveryDemandTaskIds: undefined,
    transientRecoveryStartedAt: undefined,
    transientRecoveryDeadlineAt: undefined,
    transientRecoveryTotalRequeues: 0,
    transientRecoveryConsecutiveSameSignature: 0,
    transientRecoveryLastFailureKind: undefined,
    transientRecoveryLastFailureClass: undefined,
    transientRecoveryLastFailureSignature: undefined,
    transientRecoveryLastFailureObservedAt: undefined,
    transientRecoveryLastFailureStoryId: undefined,
    transientRecoveryLastProgressReason: undefined,
    transientRecoveryLastDelayMs: undefined,
    transientRecoveryNextEligibleAt: undefined,
    transientRecoveryStoppedAt: undefined,
    transientRecoveryStopReason: undefined,
    transientRecoveryLastRequeuedStoryId: undefined,
    transientRecoveryLastHadObjectiveProgress: undefined,
    agentContextRecoveryStartedAt: undefined,
    agentContextRecoveryDeadlineAt: undefined,
    agentContextRecoveryTotalRequeues: 0,
    agentContextRecoveryLastSignature: undefined,
    agentContextRecoveryLastRequeuedStoryId: undefined,
    agentContextRecoveryStoppedAt: undefined,
    agentContextRecoveryStopReason: undefined,
    mergeRepairRecoveryStartedAt: undefined,
    mergeRepairRecoveryDeadlineAt: undefined,
    mergeRepairRecoveryTotalRequeues: 0,
    mergeRepairRecoveryConsecutiveNoProgress: 0,
    mergeRepairRecoveryLastConflictSignature: undefined,
    mergeRepairRecoveryLastProbeMessage: undefined,
    mergeRepairRecoveryLastProgressReason: undefined,
    mergeRepairRecoveryStoppedAt: undefined,
    mergeRepairRecoveryStopReason: undefined,
  };
}
